    ...
    config = function()
      ...
      local function foreach_saved_buffer(fn, current_buf_delete)
        ...
        for _, buf in ipairs(vim.api.nvim_list_bufs()) do
          if vim.bo[buf].buflisted                         -- :ls に載る普通のバッファ
      ...
      local function wipeout_all_saved()
        foreach_saved_buffer(function(buf) bufremove.wipeout(buf) end, false)
      end

      local function delete_all_saved()
        -- foreach_saved_buffer(function(buf) bufremove.delete(buf) end, false)
        foreach_saved_buffer(function(buf) bufremove.wipeout(buf) end, true)
      end


      map("n", "<leader>ba", wipeout_all_saved, { desc = "保存済みバッファをすべて消去（現在以外）" })
      map("n", "<leader>bA", delete_all_saved, { desc = "保存済みバッファをすべて消去（すべて）" })
    end,
  },
}
